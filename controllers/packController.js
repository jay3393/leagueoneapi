...
const deconstructTimelineData = function(data) {
    ...
    const validTypes = [
        ...
        "ITEM_DESTROYED", // item consumed or item combined to create another item or item destroyed like wards
        "ITEM_UNDO",      // item undo
        "ITEM_SOLD"       // item sold
    ];
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        // FIX VALID ITEMS TO INCLUDE FULL ITEMS ONLY
        // ADD A STARTER ITEMS LIST AS WELL
        const startingItems = [
            1083, 1082, 1055, 1056,
            1054, 1035, 1039, 3858,
            3862, 3850, 3854, 3070,
        ...
        const legendary = [
            3003, 3004, 3011, 3026, 3031, 3033, 
            3036, 3040, 3041, 3042, 3046, 3050, 
            3053, 3065, 3071, 3072, 3074, 3075, 
            3083, 3085, 3089, 3091, 3094, 3095, 
            3100, 3102, 3107, 3109, 3110, 3115, 
            3116, 3119, 3121, 3124, 3135, 3139, 
            3142, 3143, 3156, 3157, 3165, 3179, 
            3181, 3193, 3222, 3504, 3508, 3742, 
            3748, 3814, 3853, 3857, 3860, 3864, 
            4401, 4628, 4629, 4637, 4643, 4645, 
            6035, 6333, 6609, 6616, 6675, 6676, 
            6694, 6695, 6696, 8001, 8020, 3153
        ];

        const validItems = startingItems + mythics + boots + legendary;
        ...
        for (let j = 0; j < 10; j++) {
            if (i == 1) participants[j].startItems = (frameStack[j].filter(x => startingItems.includes(x)).sort((a, b) => a - b)).join('');
                ...
                frameStack[j].forEach(x => {
                    ...
                    else if (legendary.includes(x)) participants[j].itemPath.push(x.toString());
                });
            }
        }
    }
    ...
    participants.forEach(p => p.itemPath = p.itemPath.join(''));
