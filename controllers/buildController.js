function deconstructMatchData(data) {
    ...
    for (let i of matchParticipants) {
        ...
        const pack = {
            ...
            primarySelections: primaryStyle.selections.map(selection => selection.perk),
            subSelections: subStyle.selections.map(selection => selection.perk),
        }
...
function deconstructTimelineData(data) {
    const { metadata: { matchId }, info: { frames } } = data;

    const participants = new Array(10).fill().map(e => ({ skillPath: [], itemPath: [], startItems: []}));
        ...
        for (let event of events) {
            if (validTypes.includes(event.type)) {
                ...
                if (type === 'SKILL_LEVEL_UP') {
                    participants[participantId - 1].skillPath.push([event.levelUpType, event.skillSlot]);
                }
                else {
